...
/**
 * Skeleton loader for shipping methods page
 * Fully responsive - no overflow on any screen size
 */
export const ShippingListSkeleton: React.FC = () => {
  return (
    <div className="space-y-3 animate-pulse">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2 w-full lg:w-auto">
          <div className="h-10 w-full sm:w-32 bg-gray-300 dark:bg-gray-700 rounded"></div>
        </div>
      </div>
                ...
                <tr key={index} className="border-b border-gray-200 dark:border-gray-700">
                  {[...Array(7)].map((_, i) => (
                    <td key={i} className="px-4 py-3">
                      <div className={`h-4 ${i === 0 ? 'w-32' : i === 6 ? 'w-20' : 'w-16'} bg-gray-200 dark:bg-gray-700 rounded`}></div>
                    </td>
                  ))}
                </tr>
              ))}
