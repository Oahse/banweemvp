...
import { HomeDataResponse } from '@/api/products';
...
useEffect(() => {
  if (homeData?.data) {
    ...
    const { categories: categoriesData, featured, popular, deals: dealsData } = payload;

    // Helper function to convert API categories to demo format
    const convertCategory = (category: { id: string; name: string; image_url?: string }) => ({
      id: category.id,
      name: category.name,
